...
namespace Taum
{
    ...
    internal static class ObjMesh
    {
        public static ModelData Load(string path)
        {
            if (!File.Exists(path))
            {
                TaumPlugin.Log.LogWarning("No model at " + path);
                return null;
            }
            ...
            {
                TaumPlugin.Log.LogError("Could not read " + path + ": " + e.Message);
                return null;
            }
        }
        ...
        private static ModelData Parse(string[] lines, string name)
        {
            ...
            {
                TaumPlugin.Log.LogError("Model " + name + " has no geometry.");
                return null;
            }
            ...
            if (outPositions.Count > 65535)
            {
                ...
                TaumPlugin.Log.LogInfo(name + " needs 32 bit indices ("
            ...
            TaumPlugin.Log.LogInfo(string.Format("Loaded {0}: {1} verts, {2} tris, parts [{3}]",

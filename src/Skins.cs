...
namespace Taum
{
    ...
    internal static class Skins
    {
        ...
        private static readonly Dictionary<string, string[]> Donors =
            ...
            {
                // Leather, and it comes off a creature rather than a prop. The Lox
                // carries testloxsaddle_m on its saddle child - tack painted as tack,
                // which no crate or chest in the game is. Deer and boar hide are the
                // fallbacks: same family of colour, and both are always loaded.
                { "hide",     new[] { "Lox", "Deer", "Boar" } },
                { "darkhide", new[] { "Lox", "Deer", "Boar" } },

                // Buckles and cheek plates. A cauldron is the smallest lump of worked
                // iron the game models, and it is in every base, so it is loaded
                // wherever a halter is being looked at.
                { "iron",     new[] { "piece_cauldron", "forge", "smelter" } },
            };
        ...
        private static Material Pick(string group, bool mustEmit)
        {
            foreach (var name in DonorsFor(group))
            {
                ...
                foreach (var renderer in donor.GetComponentsInChildren<Renderer>(true))
                {
                    ...
                    TaumPlugin.Log.LogInfo(string.Format(
            ...
            TaumPlugin.Log.LogWarning("No material found for group '" + group + "'.");
            Cache[group] = null;
            return null;
        }
        ...
        {
            // Config first, so a donor that turns out to read wrong in game is a line in
            // the .cfg rather than a rebuild. Falls through to the table below when the
            // setting is empty, which is the normal case.
            var configured = TaumConfig.DonorsFor(group);
            if (configured != null && configured.Length > 0) return configured;

            string[] donors;
            return Donors.TryGetValue(group, out donors) ? donors : Donors["hide"];
        }
